...
_payload_start:
    | Supervisor + IPL=0 so VIA1 (ADB autopoll, level 1) can interrupt
    | us. iotest masks at 0x2700 because its trap calls re-enable
    | interrupts internally; keytest just polls KeyMap, which is only
    | updated by the ROM's ADB completion routine when its interrupt
    | actually fires.
    move.w  #0x2000, %sr
