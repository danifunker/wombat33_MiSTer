#!/bin/bash
# build_cpu_fpu_hda.sh -- assemble a SCSI-bootable .hda that boots the
# CPU+FPU integration bench (macos_bench/cpu_fpu_tests.h).
#
# Inputs:
#   $1 -- template .hda (defaults to ~/testdisk.hda; APM + Apple_HFS @1)
#   $2 -- output .hda   (defaults to /tmp/cpufpubench.hda)
#
...
OUT="${2:-/tmp/cpufpubench.hda}"
BUILD=build
BOOT="$BUILD/boot_stub_patch.bin"
PAYLOAD="$BUILD/payload_cpu_fpu_scsi.bin"
