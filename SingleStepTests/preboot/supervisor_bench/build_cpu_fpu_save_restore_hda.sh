#!/bin/bash
# build_cpu_fpu_save_restore_hda.sh -- assemble a SCSI-bootable .hda that boots the
# FSAVE/FRESTORE sub-corpus bench (macos_bench/save_restore_tests.h).
#
# Inputs:
#   $1 -- template .hda (defaults to ~/testdisk.hda; APM + Apple_HFS @1)
#   $2 -- output .hda   (defaults to /tmp/cpufpu_saverestore.hda)
#
...
OUT="${2:-/tmp/cpufpu_saverestore.hda}"
BUILD=build
BOOT="$BUILD/boot_stub_patch.bin"
PAYLOAD="$BUILD/payload_cpu_fpu_save_restore_scsi.bin"
