#!/bin/bash
# build_cpu_fpu_baseline_hda.sh -- assemble a SCSI-bootable .hda that boots the
# CPU+FPU baseline-only bench (1328 math tests, NO FSAVE/FRESTORE).
#
# Inputs:
#   $1 -- template .hda (defaults to ~/testdisk.hda; APM + Apple_HFS @1)
#   $2 -- output .hda   (defaults to /tmp/cpufpubench_baseline.hda)
#
...
OUT="${2:-/tmp/cpufpubench_baseline.hda}"
BUILD=build
BOOT="$BUILD/boot_stub_patch.bin"
PAYLOAD="$BUILD/payload_cpu_fpu_baseline_scsi.bin"
